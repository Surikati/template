 ...
 * and a small whitelist of pure functions ({@code len}, {@code empty}, {@code today}, {@code
 * formatDate}). No reflection, no method calls on user data — this is a safe evaluator for
...
public class AntlrExpressionEvaluator implements ExpressionEvaluator {
  ...
  private static final class EvaluatingVisitor extends TmpExprBaseVisitor<Object> {
    ...
    public Object visitUnary(TmpExprParser.UnaryContext ctx) {
      if (ctx.unary() != null) {
        ...
        throw new ExpressionException("expression.type_error", "Unary minus on non-number: " + v);
      }
    ...
    public Object visitLiteral(TmpExprParser.LiteralContext ctx) {
      ...
      return switch (text) {
        ...
        case "null" -> null;
        default ->
            throw new ExpressionException("expression.unknown_literal", "Unknown literal: " + text);
      };
    }
    ...
    private static int cmp(Object a, Object b) {
      ...
      throw new ExpressionException(
          "expression.type_error", "Cannot compare " + typeOf(a) + " and " + typeOf(b));
    }
    ...
    private static Object arith(Object a, Object b, String op) {
      ...
      double r =
          switch (op) {
            ...
            case "/" -> x / y;
            default ->
                throw new ExpressionException("expression.unknown_op", "Unknown operator: " + op);
          };
    ...
    /**
     * {@code value in collection} — true when collection contains an element equal to value under
     * our {@link #eq} semantics. Right side must be a Collection (typically a list literal or a
     * path resolving to a list); other shapes throw a type error.
     */
  ...
  private static final class Functions {
    ...
    private static long len(Object v) {
      ...
      throw new ExpressionException("expression.type_error", "len() on unsupported type: " + v);
    }

    private static String formatDate(Object dateValue, String pattern) {
      if (dateValue == null) return "";
      return toLocalDate(dateValue, "formatDate").format(DateTimeFormatter.ofPattern(pattern));
    }
    ...
    private static LocalDate toLocalDate(Object value, String fnName) {
      ...
      throw new ExpressionException(
          "expression.type_error", fnName + " expects ISO string or LocalDate, got " + value);
    }
  }
}
