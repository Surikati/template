...
/**
 * Polls the {@code outbox_event} table and publishes unsent events to RabbitMQ.
 * Uses SELECT … FOR UPDATE SKIP LOCKED so multiple instances can run concurrently.
 */
@Component
public class OutboxPublisher {
    ...
    public void drain() {
        ...
        for (OutboxEvent evt : batch) {
            ...
            rabbit.convertAndSend(props.exchange(), routingKey, evt.payload(), m -> {
