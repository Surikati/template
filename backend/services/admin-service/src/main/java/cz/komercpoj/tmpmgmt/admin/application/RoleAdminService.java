...
public class RoleAdminService {
  ...
  public RoleAdminService(
      AppUserRepository users, RoleDefinitionRepository roles, UserRoleRepository assignments) {
    this.users = users;
    this.roles = roles;
  ...
  public UserRoleEntity grant(UUID userId, String roleCode, UUID grantedBy) {
    ...
    if (!roles.existsById(roleCode)) {
      throw new NotFoundException("role.not_found", "Role code unknown: " + roleCode);
    }
