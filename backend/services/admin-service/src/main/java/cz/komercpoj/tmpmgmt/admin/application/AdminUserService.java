...
public class AdminUserService {
    ...
    public AdminUserService(
            AppUserRepository users, Keycloak keycloak, KeycloakProperties props) {
        this.users = users;
    ...
    public SyncResult sync() {
        ...
        for (UserRepresentation kc : kcUsers) {
            ...
            if (existing.isEmpty()) {
                users.save(AppUserEntity.createNew(kc.getId(), kc.getUsername(),
                        kc.getEmail(), displayName));
                created++;
            } else {
                AppUserEntity e = existing.get();
                e.refreshFromKeycloak(kc.getUsername(), kc.getEmail(), displayName,
                        Boolean.TRUE.equals(kc.isEnabled()));
                updated++;
            }
        }
        log.info("Keycloak sync: {} created, {} updated (total fetched: {})",
                created, updated, kcUsers.size());
    ...
    public AppUserEntity getById(UUID id) {
        return users.findById(id).orElseThrow(() ->
                new NotFoundException("user.not_found", "User not found: " + id));
    }
