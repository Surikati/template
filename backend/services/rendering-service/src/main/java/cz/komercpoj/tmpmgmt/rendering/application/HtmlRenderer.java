...
/**
 * HTML preview variant of {@link DocxRenderer}. Walks the same AST and emits sanitized,
 * styled HTML for on-screen preview. Values from text/variable nodes are HTML-escaped.
 * No script/style tags are ever emitted; the FE can drop the output into a sanitized div.
 */
@Component
    ...
    private void walkBlocks(JsonNode blocks, Map<String, Object> data, StringBuilder sb) {
        ...
        while (it.hasNext()) {
            ...
            switch (type) {
                ...
                default -> { /* unknown block types silently skipped */ }
            }
        }
    }
