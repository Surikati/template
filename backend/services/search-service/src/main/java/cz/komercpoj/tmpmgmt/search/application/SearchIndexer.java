...
/**
 * Listens for template and clause domain events and keeps the OpenSearch indices in sync. Created
 * events carry the full indexable metadata (description, category, tags) so this service stays
 * event-driven without follow-up API fetches. Subsequent {@code version.published} and {@code
 * archived} events do narrow partial updates (timestamp / status only).
 */
@Component
public class SearchIndexer {
  ...
  private void handle(Message message, String index, String idField) {
    String eventType =
        String.valueOf(message.getMessageProperties().getHeaders().get("event-type"));
    JsonNode payload;
    try {
  ...
  private void applyMetadataUpdate(String index, UUID id, JsonNode payload) throws Exception {
    ...
    client.update(
        UpdateRequest.of(u -> u.index(index).id(id.toString()).doc(partial).docAsUpsert(false)),
        Map.class);
  }

  private void upsertFromCreated(String index, UUID id, JsonNode payload) throws Exception {
    SearchableDocument doc =
        new SearchableDocument(
            ...
            occurredAt(payload));
    client.update(
        UpdateRequest.of(u -> u.index(index).id(id.toString()).doc(doc).docAsUpsert(true)),
  ...
  private void setStatus(String index, UUID id, String status, JsonNode payload) throws Exception {
    Map<String, Object> partial = Map.of("status", status, "updatedAt", occurredAt(payload));
    client.update(
        UpdateRequest.of(u -> u.index(index).id(id.toString()).doc(partial).docAsUpsert(false)),
        Map.class);
  }
