...
/**
 * Expands {@code clauseRef} nodes in a template AST by fetching the referenced clause version
 * from clause-service and inlining its fragment content in place. A clauseRef is a block node
 * that gets replaced by zero or more block nodes.
 *
 * <p>Recursion descends into {@code content} arrays so clauseRefs inside {@code conditionBlock},
 * {@code repeatBlock}, and nested clauses are all resolved. The resolver walks the expanded
 * content too, so clauses referencing other clauses are handled transitively — subject to a
 * cycle guard (max depth) to avoid stack overflow.
 */
@Component
public class ClauseResolver {
    ...
    /**
     * Copies the given container node, replacing its {@code content} array with the
     * resolved (expanded) children. Returns the copy — inputs are never mutated.
     */
    ...
    private void expandClauseRef(JsonNode refNode, ArrayNode target, int depth) {
        if (depth >= MAX_DEPTH) {
            throw new DomainException(
                    ...
                    "Clause reference resolution exceeded depth " + MAX_DEPTH
                            + " — likely a cycle.");
        }
        ...
        if (clauseIdText == null || versionNumber <= 0) {
            throw new DomainException(
                    "clause.ref_invalid",
                    "clauseRef missing clauseId or versionNumber: " + refNode);
        }
        ...
        try {
            ...
            throw new DomainException(
                    "clause.ref_invalid",
                    "clauseRef.clauseId is not a UUID: " + clauseIdText);
        }
