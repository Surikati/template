...
class AuditConsumerIT {
    ...
    void consumesEventFromExchange_storesAndQuery() throws Exception {
        ...
        rabbit.convertAndSend(auditExchange.getName(), "template.created",
        ...
        await().atMost(Duration.ofSeconds(10)).untilAsserted(() -> {
            List<?> events = repo.findByAggregate(
        ...
        // Verify fields
        var saved = repo.findByAggregate("template", templateId.toString(),
    ...
    void duplicateMessage_isIdempotent() throws Exception {
        ...
        for (int i = 0; i < 2; i++) {
            rabbit.convertAndSend(auditExchange.getName(), "clause.created", json,
                    m -> {
                        m.getMessageProperties().setMessageId(eventId.toString());
                        m.getMessageProperties().setHeader("event-type", "created");
        ...
        await().atMost(Duration.ofSeconds(10)).untilAsserted(() ->
                assertThat(repo.findByAggregate("clause", clauseId.toString(),
                        org.springframework.data.domain.PageRequest.of(0, 10)))
                        .hasSize(1));
    }
}
