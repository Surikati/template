 ...
 * the partitioned {@code audit_event} table. Idempotent — duplicates are skipped on {@code
 * (event_id, occurred_at)} conflict.
 */
@Component
public class AuditEventConsumer {
  ...
  /**
   * Payload fields we probe (in order) to infer the acting user. Falls back to {@code null} when no
   * identifiable actor is present (system-emitted events).
   */
  private static final List<String> ACTOR_CANDIDATES =
      List.of(
          "actorUserId",
          "publishedBy",
          "ownerUserId",
