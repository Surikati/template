...
/**
 * Full upload → MinIO → download path. Uses a GenericContainer for MinIO
 * (there's no official Testcontainers module for it).
 */
@SpringBootTest
@ActiveProfiles("test")
...
class DocumentLifecycleIT {
    ...
    static void minioProps(DynamicPropertyRegistry registry) {
        registry.add("tmpmgmt.minio.endpoint",
    ...
    void uploadsToMinio_persistsMetadata_emitsEvent_andStreamsBack() throws Exception {
        ...
        var result = service.upload(new DocumentCommands.UploadDocument(
                templateId, 1, jobId,
        ...
        assertThat(events).extracting(r -> r.get("event_type"))
