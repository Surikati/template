...
public class DocumentService {
    ...
    public GeneratedDocumentEntity upload(UploadDocument cmd) {
        ...
        GeneratedDocumentEntity doc = GeneratedDocumentEntity.create(
                id,
                cmd.templateId(),
        ...
        for (FileInput fi : cmd.files()) {
            ...
            FileReferenceEntity fr = FileReferenceEntity.create(
                    UUID.randomUUID(), fi.format(), key, fi.content().length, sha);
            doc.addFile(fr);

            eventFiles.add(new DocumentEvents.DocumentGenerated.FileRef(
                    fi.format(), fi.content().length, sha));
        }
    ...
    public GeneratedDocumentEntity getById(UUID id) {
        return docs.findById(id)
                .orElseThrow(() -> new NotFoundException(
                        "document.not_found", "Document not found: " + id));
    }
    ...
    public GetObjectResponse download(FileReferenceEntity ref) {
        try {
            return minio.getObject(GetObjectArgs.builder()
                    ...
                    .object(ref.getMinioKey())
                    .build());
        } catch (Exception e) {
            throw new DomainException(
                    "document.minio_read_failed",
                    "Failed to read " + ref.getMinioKey() + ": " + e.getMessage(), e);
        }
    }
    ...
    private void putToMinio(String key, byte[] content, FileFormat format) {
        try {
            ...
            throw new DomainException(
                    "document.minio_write_failed",
                    "Failed to store " + key + ": " + e.getMessage(), e);
        }
    }
