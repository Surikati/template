...
public class DocumentController {
    ...
    public ResponseEntity<DocumentResponse> upload(
        ...
        List<DocumentCommands.FileInput> fileInputs = req.files().stream()
                .map(this::decode)
                .toList();
        var doc = service.upload(new DocumentCommands.UploadDocument(
                req.templateId(),
    ...
    private DocumentCommands.FileInput decode(FileInputDto dto) {
        return new DocumentCommands.FileInput(dto.format(), Base64.getDecoder().decode(dto.contentBase64()));
    }

    private DocumentResponse toResponse(GeneratedDocumentEntity doc) {
        List<FileReferenceResponse> files = doc.getFiles().stream()
                .map(f -> new FileReferenceResponse(
                        f.getId(),
                        f.getFormat(),
                        f.getSizeBytes(),
                        f.getSha256(),
