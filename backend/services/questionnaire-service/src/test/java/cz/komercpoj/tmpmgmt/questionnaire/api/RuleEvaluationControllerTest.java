...
class RuleEvaluationControllerTest {
  ...
  void perRuleErrorIsCaptured_othersStillEvaluate() {
    var req =
        new EvaluateRulesRequest(
            List.of(new RuleInput("good", "x == 5"), new RuleInput("bad-syntax", "x ===== 5")),
            Map.of("x", 5));
  ...
  void missingPathTreatedAsFalsy_noError() {
    var req =
        new EvaluateRulesRequest(List.of(new RuleInput("k", "missing.field == 'x'")), Map.of());
