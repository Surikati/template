...
public class SessionService {
  ...
  public QuestionnaireSessionEntity start(StartSession cmd) {
    if (!questionnaires.existsById(cmd.questionnaireId())) {
      throw new NotFoundException(
          "questionnaire.not_found", "Questionnaire not found: " + cmd.questionnaireId());
    }
    UUID id = UUID.randomUUID();
    QuestionnaireSessionEntity s =
        QuestionnaireSessionEntity.start(
  ...
  public QuestionnaireSessionEntity submitAnswers(SubmitAnswers cmd) {
    ...
    if (s.getState() != SessionState.IN_PROGRESS) {
      throw new ConflictException(
          "session.not_in_progress", "Session is " + s.getState() + ", cannot submit answers.");
    }
    ObjectNode merged =
        s.getAnswers().deepCopy().isObject()
  ...
  public QuestionnaireSessionEntity complete(CompleteSession cmd) {
    ...
    if (s.getState() != SessionState.IN_PROGRESS) {
      throw new ConflictException(
          "session.not_in_progress", "Session is already " + s.getState() + ".");
    }
    s.markCompleted();
    ...
    outbox.stage(
        ...
        new QuestionnaireEvents.SessionCompleted(
            s.getId(), s.getQuestionnaireId(), s.getStartedBy(), s.getAnswers(), Instant.now()));
    return s;
  }
