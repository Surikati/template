...
public class TemplateService {
    ...
    public TemplateEntity create(CreateTemplate cmd) {
        if (templates.existsBySlug(cmd.slug())) {
            throw new ConflictException(
                    "template.slug_taken", "Slug already in use: " + cmd.slug());
        }
        UUID id = UUID.randomUUID();
        TemplateEntity t = TemplateEntity.newActive(
        ...
        JsonNode emptyDoc = mapper.createObjectNode()
                .put("type", "doc")
                .set("content", mapper.createArrayNode().add(
                        mapper.createObjectNode().put("type", "paragraph")));
        JsonNode emptySchema = mapper.createObjectNode()
    ...
    public TemplateDraftEntity getDraft(UUID templateId) {
        return drafts.findById(templateId)
    ...
    public TemplateDraftEntity saveDraft(SaveDraft cmd) {
        ...
        TemplateDraftEntity draft = drafts.findById(cmd.templateId())
                .orElseGet(() -> TemplateDraftEntity.empty(
                        cmd.templateId(), cmd.content(), cmd.variablesSchema(), cmd.editorUserId()));
        ...
        outbox.stage(
                ...
                new TemplateEvents.TemplateDraftSaved(
                        template.getId(), cmd.editorUserId(), Instant.now()));
        return draft;
    }
    ...
    public TemplateVersionEntity publishVersion(PublishVersion cmd) {
        ...
        TemplateVersionEntity v = TemplateVersionEntity.publish(
                versionId,
                template.getId(),
                nextVersion,
                draft.getContent(),
    ...
    public void archive(Archive cmd) {
        ...
        outbox.stage(
                ...
                new TemplateEvents.TemplateArchived(
                        template.getId(), cmd.actorUserId(), Instant.now()));
    }
