...
public class ClauseService {
  ...
  public ClauseEntity create(CreateClause cmd) {
    if (clauses.existsBySlug(cmd.slug())) {
      throw new ConflictException("clause.slug_taken", "Slug already in use: " + cmd.slug());
    }
    UUID id = UUID.randomUUID();
    ClauseEntity c =
        ClauseEntity.newActive(
  ...
  public ClauseVersionEntity getVersion(UUID clauseId, int versionNumber) {
    return versions
        .findByClauseIdAndVersionNumber(clauseId, versionNumber)
        .orElseThrow(
            () ->
  ...
  public ClauseVersionEntity publishVersion(PublishVersion cmd) {
    ...
    ClauseVersionEntity v =
        ClauseVersionEntity.publish(
            versionId,
            clause.getId(),
            nextVersion,
            cmd.content(),
  ...
  public void archive(Archive cmd) {
    ...
    outbox.stage(
        ...
        new ClauseEvents.ClauseArchived(clause.getId(), cmd.actorUserId(), Instant.now()));
  }
