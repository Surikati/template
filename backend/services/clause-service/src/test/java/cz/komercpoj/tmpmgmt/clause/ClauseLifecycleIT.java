...
class ClauseLifecycleIT {
    ...
    void fullLifecycle_stagesExpectedEvents_andEnforcesVersionImmutability() {
        ...
        var v1 = service.publishVersion(new ClauseCommands.PublishVersion(
                clauseId, content, "initial release", actor));
        assertThat(v1.getVersionNumber()).isEqualTo(1);

        // 3. Publish v2
        ObjectNode content2 = content.deepCopy();
        content2.put("revision", 2);
        var v2 = service.publishVersion(new ClauseCommands.PublishVersion(
                clauseId, content2, "minor update", actor));
    ...
    void createWithDuplicateSlug_throwsConflict() {
        service.create(new ClauseCommands.CreateClause(
                "liability", "Odpovědnost", null, "legal", actor));
        assertThatThrownBy(() -> service.create(new ClauseCommands.CreateClause(
    ...
    void publishWithInvalidContent_throwsValidation() {
        var c = service.create(new ClauseCommands.CreateClause(
                "bad-content", "Bad", null, null, actor));
        ...
        assertThatThrownBy(() -> service.publishVersion(new ClauseCommands.PublishVersion(
                c.getId(), wrong, null, actor)))
