...
import { ClauseApiService, ClauseVersionResponse } from '@tmpmgmt/api-client';
import { ProblemDetail } from '@tmpmgmt/core';

@Component({
  selector: 'tm-publish-clause-version-dialog',
  standalone: true,
  changeDetection: ChangeDetectionStrategy.OnPush,
  imports: [ReactiveFormsModule, DialogModule, InputTextareaModule, ButtonModule],
  template: `
    <p-dialog
      header="Publikovat verzi doložky"
      [visible]="visible()"
    ...
    >
      ...
      <ng-template pTemplate="footer">
        ...
        <p-button
          label="Publikovat"
          icon="pi pi-upload"
          [disabled]="form.invalid || submitting() || !content()"
          [loading]="submitting()"
          (onClick)="submit()"
        />
      </ng-template>
...
export class PublishClauseVersionDialogComponent {
  private readonly api = inject(ClauseApiService);
  private readonly fb = inject(FormBuilder);
  private readonly messages = inject(MessageService);

  readonly visible = input(false);
  readonly clauseId = input.required<string>();
  /** Fragment-rooted AST captured from the editor at open time. */
  readonly content = input<unknown>(null);

  readonly visibleChange = output<boolean>();
  readonly published = output<ClauseVersionResponse>();
  ...
  protected submit(): void {
    if (this.form.invalid || this.submitting() || !this.content()) return;
    this.submitting.set(true);
    this.api
      .publishVersion(this.clauseId(), {
